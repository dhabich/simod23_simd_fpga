...
uint32_t filter_kernel(T in, size_t size, U lower, U upper) {
    using namespace tvl;
...
double filter_fpga(queue* q, Type* in_host, long* out_host, size_t size) {
    auto start = high_resolution_clock::now();
    q->submit([&](handler& h) {
         h.single_task([=]() [[intel::kernel_args_restrict]] {
             host_ptr<Type> in(in_host);
             host_ptr<long> out(out_host);
             out[0] = filter_kernel<MyVec>(in, size, static_cast<float>(lower_bound), static_cast<float>(upper_bound));
         });
     }).wait();
...
double filter_host(T in, size_t size) {
    double speed = 0;
    volatile size_t results = 0;
    for (size_t its = 0; its < 10; ++its) {
        auto start = std::chrono::high_resolution_clock::now();
        results += filter_kernel<MyVec>(in, size, lower_bound, upper_bound);
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        pcie_time[0] = filter_fpga<tvl::simd<Type, tvl::fpga, 128>>(&q, in, out_aggr, vec_elems);

        // Actual measurement can start
        std::cout << "Running HOST-Filter test with " << size << " values" << std::endl;
        pcie_time[0] = filter_fpga<tvl::simd<Type, tvl::fpga, 128>>(&q, in, out_aggr + 0, number_CL * vec_elems);
        pcie_time[1] = filter_fpga<tvl::simd<Type, tvl::fpga, 256>>(&q, in, out_aggr + 1, number_CL * vec_elems);
        pcie_time[2] = filter_fpga<tvl::simd<Type, tvl::fpga, 512>>(&q, in, out_aggr + 2, number_CL * vec_elems);
        pcie_time[3] = filter_fpga<tvl::simd<Type, tvl::fpga, 1024>>(&q, in, out_aggr + 3, number_CL * vec_elems);
        pcie_time[4] = filter_fpga<tvl::simd<Type, tvl::fpga, 2048>>(&q, in, out_aggr + 4, number_CL * vec_elems);
    } catch (exception const& e) {
    ...
    size_t res1 = filter_host<tvl::simd<Type, tvl::sse>>(in, (number_CL * vec_elems));
    size_t res2 = filter_host<tvl::simd<Type, tvl::avx2>>(in, (number_CL * vec_elems));
    size_t res3 = filter_host<tvl::simd<Type, tvl::avx512>>(in, (number_CL * vec_elems));
